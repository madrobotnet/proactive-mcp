...
- **에이전트 의존 MCP, 호스트 중립 구현.** proactive-mcp에는 에이전트나 모델이 없으며 Grok·Codex·Hermes 또는 다른 호스트를 시작하지 않는다. 이미 실행 중이거나 호스트가 예약한 에이전트가 MCP 도구를 명시적으로 호출해야 전달이 일어난다. 구현은 특정 호스트 adapter에 종속되지 않는다.
...
flowchart LR
    subgraph agents [에이전트들]
        A1[실행 중인 host agent]
        A2[host-native scheduled agent run]
        A3[수동 restricted agent run]
    end
...
| 프로세스 | 실행 방식 | 역할 |
|---|---|---|
| `proactive-mcp serve` | host MCP client가 stdio로 spawn (다중 인스턴스 허용) | 일상 MCP 도구 표면 제공. DB 읽기/쓰기. host agent/model은 시작하지 않음 |
| `proactive-mcp serve-scheduled` | host MCP client가 별도 restricted 대화에서 stdio로 spawn | `get_status`·`proactive_check`·`confirm_delivery`만 제공. scheduler/collector launcher가 아니며 스스로 도구를 호출하지 않음 |
| `proactive-mcp daemon` | 상시 실행 (systemd / Windows 작업 스케줄러 / 수동) | 주기적 Google sync, Situation 평가, queue 유지, 문서화된 폴백 알림. agent/LLM 호출·prompt 전송 금지 |
| `proactive-mcp setup` | 1회성 CLI | Google OAuth 연동, 초기 설정 |
| `proactive-mcp status` | 1회성 CLI | 연결·신선도·데몬 상태·누적 전달 수 진단 |

**데몬 없는 degraded 모드:** 데몬이 꺼져 있어도 실행 중인 host agent가 `proactive_check`를 명시적으로 호출하면 마지막 sync가 오래되었을 때 인라인으로 lazy sync 후 평가한다. 이 경우 폴백 알림은 동작하지 않으며 `get_status`가 이를 명시한다. 데몬은 권장 사항이지 필수가 아니다. 반대로 데몬이나 `serve-scheduled`만 실행 중이고 host agent가 꺼져 있으면 conversation/delivery는 시작되지 않으며 pending situation은 다음 명시적 MCP 호출까지 유지된다.
...
2. **실행 중인 에이전트** — MCP는 agent/model/conversation을 생성하지 않는다. 에이전트가 꺼져 있으면 plugin 단독 전달은 없다.
3. **immutable dedicated per-run MCP profile** — 자동 예약 run에는 `serve-scheduled`만 로드되고 일상 `serve`가 없는 별도 profile이 필요하다. Host/version이 이를 제공하지 못하면 자동 사용은 예약하지 않는 방식으로 fail closed한다. 수동 restricted 대화는 가능하다.

스케줄 트리거는 host-native scheduler 또는 OS cron이 **host agent run**을 시작하는 host/operator 기능이다. proactive-mcp의 daemon과 `serve-scheduled`는 trigger가 아니다.
