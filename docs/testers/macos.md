...
- 이 에이전트에 MCP를 등록하세요. 일상 대화에는 serve만 로드하고 별도 예약 대화에는 serve-scheduled만 로드하세요. 한 대화에 두 프로필을 함께 로드하지 마세요. 등록에는 ~/를 쓰지 말고 /Users/내-사용자명/venvs/proactive/bin/proactive-mcp 같은 절대 경로를 쓰세요.
