...
Owner 쪽에는 Mac이 없습니다. Gatekeeper나 알림 권한 창이 뜨면 그 문구만 적어 주세요. 메일 내용은 적지 마세요.
...
파일을 커밋하거나 채팅에 붙여 넣지 마세요.

## 에이전트에게 붙여 넣기

```text
이 Mac에 proactive-mcp 클로즈드 알파를 설치하고, 지금 이 에이전트에 MCP로 등록해 주세요. 내가 명령을 외우거나 grok mcp add / codex mcp add를 직접 치우지 않게 해 주세요.

규칙:
- 저장소 clone 금지. PyPI, uvx, pip install proactive-mcp 금지. PROACTIVE_DATABASE 설정 금지.
- wheel: ~/Downloads/proactive_mcp-0.1.0-py3-none-any.whl
- 다른 채널로 받은 SHA-256과 shasum -a 256으로 비교하고, 다르면 설치를 멈추세요.
- OAuth JSON이 있으면 ~/.proactive-mcp/client_secret.json으로 옮기고 디렉터리 700, 파일 600으로 두세요. 없으면 내가 BYO인지 확인하고 SETUP_GOOGLE.md를 안내하세요.
- Python 3.11과 uv가 없으면 설치하세요. 전용 venv(~/venvs/proactive)에 wheel만 넣으세요.
- 이 에이전트에 등록하세요. 일상용은 serve, 스케줄용은 serve-scheduled입니다. 설정에는 물결표 없이 절대 경로를 넣으세요.
- setup을 실행하세요. Google 경고가 나면 내가 Advanced → Continue를 누릅니다. 성공 줄은 Google read-only sources configured. 입니다.
...
```text
proactive-mcp를 이 Mac에서 제거해 주세요. 에이전트 MCP 등록을 빼고, 자격 증명을 먼저 삭제한 뒤에만 ~/.proactive-mcp와 ~/venvs/proactive를 지우세요. 메일 내용과 토큰은 출력하지 마세요.
```
