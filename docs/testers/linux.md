...
```text
이 Linux PC에 proactive-mcp 클로즈드 알파를 설치하고, 지금 이 에이전트에 MCP로 등록해 주세요. 내가 명령을 외우거나 grok mcp add / codex mcp add를 직접 치우지 않게 해 주세요.
...
- 다른 채널로 받은 SHA-256과 sha256sum으로 비교하고, 다르면 설치를 멈추세요.
...
```text
proactive-mcp를 이 PC에서 제거해 주세요. 에이전트 MCP 등록을 빼고, 자격 증명을 먼저 삭제한 뒤에만 ~/.proactive-mcp와 ~/venvs/proactive를 지우세요. 메일 내용과 토큰은 출력하지 마세요.
```
