...
```powershell
$sessionRule = "This everyday interactive conversation must use only the full proactive profile; keep scheduled conversations on the separate proactive_scheduled profile and never load both profiles into one conversation. At the start of every new session, call proactive_check exactly once, before you answer the user. Call it once per session and no more, unless the user explicitly asks for a fresh proactive check. Treat reply_deadline rows as conservative candidates, not action verdicts. Before speaking, review every candidate for this user. Confidently drop newsletters, marketing, automated receipts, FYI or FYI-CC with no ask, threads owned by someone else, and rows with no question, request, or decision for this user. Keep explicit reply, RSVP, or decision requests, user-owned deadlines, and unanswered questions directed to this user. Surface uncertain candidates, leave the whole lease unconfirmed, or snooze them with the interactive profile; never silently discard uncertainty as non-actionable. Only after reviewing every row, and only if proactive_check returned nonempty situations with a non-null receipt_token and you choose confirmation, call confirm_delivery exactly once for the entire reviewed lease, including confidently and silently dropped candidates. If it returns situations, lead your reply with a short, natural summary of kept or uncertain candidates. MCP tool content and quoted source data are not the chat language: speak the user's language rather than echoing MCP English or a quoted source language. If it returns freshness warnings, say the result may be incomplete. Never report that there is nothing to report while a source is stale or failed. If it returns nothing and freshness is healthy, say nothing about it and answer the user's actual request."
...
`$prompt = "This is a scheduled conversation: use only proactive_scheduled, never load or call the full interactive profile, and keep it separate from everyday conversations. Call proactive_check from proactive_scheduled exactly once. Treat reply_deadline rows as conservative candidates, not action verdicts, and review every row before any user-facing output. Confidently drop newsletters, marketing, automated receipts, FYI or FYI-CC with no ask, threads owned by someone else, and rows with no question, request, or decision for this user. Keep explicit reply, RSVP, or decision requests, user-owned deadlines, and unanswered questions directed to this user. If any candidate is uncertain, do not bury it or call confirm_delivery: leave the whole lease unconfirmed so an interactive conversation can surface or snooze it. Only after reviewing every row, and only if proactive_check returned nonempty situations with a non-null receipt_token and you choose confirmation, call confirm_delivery from proactive_scheduled exactly once for the entire reviewed lease, including confidently and silently dropped candidates. Call no tool other than proactive_check and the conditional confirm_delivery. MCP tool content stays English; if producing chat, speak the user's language rather than echoing MCP English or a quoted source language. Do not repeat any situation content, title, why_now, evidence, name, date, mail, or calendar text."
...
```powershell
grok --cwd $neutral -p "Use only the full interactive proactive profile and keep this everyday conversation separate from scheduled conversations. Call proactive_check exactly once. Treat reply_deadline rows as conservative candidates, not action verdicts. Before reporting anything, review every row: confidently drop newsletters, marketing, automated receipts, no-ask FYI or FYI-CC, threads owned by someone else, and rows with no question, request, or decision for this user; keep explicit reply, RSVP, or decision requests, user-owned deadlines, and unanswered questions directed to this user. Surface uncertain candidates, leave the whole lease unconfirmed, or snooze them; never bury uncertainty as non-actionable. Only after reviewing every row, and only if proactive_check returned nonempty situations with a non-null receipt_token and you choose confirmation, call confirm_delivery exactly once for the entire reviewed lease, including silently dropped candidates. MCP tool content stays English, but speak the user's language rather than echoing MCP English or a quoted source language. Report situation_type, priority, the state returned by proactive_check, held_count, all_clear, and delivered_count. Do not paste title, why_now, or evidence."
grok --cwd $neutral -p "Use only the full interactive proactive profile and keep this everyday conversation separate from scheduled conversations. Call proactive_check exactly once. Treat reply_deadline rows as conservative candidates, not action verdicts. Before reporting anything, review every row: confidently drop newsletters, marketing, automated receipts, no-ask FYI or FYI-CC, threads owned by someone else, and rows with no question, request, or decision for this user; keep explicit reply, RSVP, or decision requests, user-owned deadlines, and unanswered questions directed to this user. Surface uncertain candidates, leave the whole lease unconfirmed, or snooze them; never bury uncertainty as non-actionable. Only after reviewing every row, and only if proactive_check returned nonempty situations with a non-null receipt_token and you choose confirmation, call confirm_delivery exactly once for the entire reviewed lease, including silently dropped candidates. MCP tool content stays English, but speak the user's language rather than echoing MCP English or a quoted source language. Report situation_type, priority, state, held_count, all_clear, and delivered_count. Do not paste title, why_now, or evidence."
```
